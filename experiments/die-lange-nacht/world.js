...
function captionHTML(o) {
  ...
  return `<div class="cap">
      ...
      ${o.story ? '<span class="more">Mehr dazu</span>' : ''}
    </div>`;
}
...
function buildObject(o) {
  ...
  if (o.kind === 'work' || o.kind === 'scene') {
    ...
    node = typeNode('o-mono', `<div class="frame">
      ...
      <span class="more">Mehr dazu</span></div>`);
  } else if (o.kind === 'end') {
    node = typeNode('o-end', `<h2>${esc(o.title)}</h2><p class="places">${esc(o.sub)}</p>
      <button class="restart" type="button">Zurück an den Anfang</button>`);
  }
  if (!node) return null;
...
function openCluster(it) {
  ...
  add('c-close', '<button type="button">Zurück in die Nacht</button>',
...
function buildReading() {
  ...
  readingBuilt = true;
  let html = `<button class="exit" type="button">Zurück in die Nacht</button>
    <p class="kicker">Inspired · Ausgabe Nr. 45</p>
    <h1>Die lange Nacht</h1>
    <p class="dek">Die vollständigen Texte und Werke dieser Ausgabe, linear und ohne Bewegung.
      Diese Ansicht ist der barrierearme Zugang zur selben Ausgabe.</p>`;
